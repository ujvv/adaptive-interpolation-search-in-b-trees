...
uint16_t BTreeNodeBinarySearchNoPrefix::getEntryIndexByKey(std::span<uint8_t> key) {
  ...
  uint32_t keyHead = getKeyHead(key);

  if (numKeys == 0 || keySmallerEqualThanAtPosition(0, keyHead, key)) {
    return 0;
  }
  if (numKeys > 0 && keyLargerThanAtPosition(numKeys - 1, keyHead, key)) {
    return numKeys;
  }
  ...
  while (left <= right) {
    ...
    if (keySmallerEqualThanAtPosition(middle, keyHead, key)) {
      childIndex = middle;
      right = middle - 1;
    } else {
      left = middle + 1;
...
void BTreeNodeBinarySearchNoPrefix::insertEntry(uint16_t position, std::span<uint8_t> key, std::span<uint8_t> value) {
  uint16_t requiredSpace = sizeof(PageSlotBinarySearchNoPrefix) + key.size() + value.size();
  ...
  freeOffset -= (key.size() + value.size());
  if (key.size() > 0) {
    std::memcpy(content + freeOffset, key.data(), key.size());
  }
  if (value.size() > 0) {
    std::memcpy(content + freeOffset + key.size(), value.data(), value.size());
  }

  // Store the size and offset information in the new slot
  auto slot = reinterpret_cast<PageSlotBinarySearchNoPrefix *>(content + position * sizeof(PageSlotBinarySearchNoPrefix));
  slot->offset = freeOffset;
  slot->keyLength = key.size();
  slot->valueLength = value.size();
  slot->keyHead = getKeyHead(key);
...
BTreeNodeBinarySearchNoPrefix *BTreeNodeBinarySearchNoPrefix::splitNode(uint16_t splitIndex) {
  // Create the new node
  BTreeNodeBinarySearchNoPrefix *newNode = nullptr;
  if (isLeaf) {
    newNode = new BTreeLeafNodeBinarySearchNoPrefix();
  } else {
    newNode = new BTreeInnerNodeBinarySearchNoPrefix();
  }
...
std::optional<std::pair<BTreeNodeBinarySearchNoPrefix *, std::vector<uint8_t>>> BTreeNodeBinarySearchNoPrefix::insert(std::span<uint8_t> key, std::span<uint8_t> value) {
  if (!isLeaf) {
    ...
    BTreeInnerNodeBinarySearchNoPrefix *newRightSibling = reinterpret_cast<BTreeInnerNodeBinarySearchNoPrefix *>(splitNode(splitIndex + 1));
    ...
    BTreeInnerNodeBinarySearchNoPrefix *newThisChild = new BTreeInnerNodeBinarySearchNoPrefix();
    ...
    if (insertIndex < numKeys && key == getShortenedKey(insertIndex)) {
    ...
    // Determine the new right sibling and the index of the last key that should remain
    // In the current node
    if (insertIndex < splitIndex) {
      splitKey = getFullKey(splitIndex - 1);
      newRightSibling = reinterpret_cast<BTreeLeafNodeBinarySearchNoPrefix *>(splitNode(splitIndex));
      lastEntryIndex = splitIndex - 1;
    } else if (insertIndex > splitIndex) {
      splitKey = getFullKey(splitIndex);
      newRightSibling = reinterpret_cast<BTreeLeafNodeBinarySearchNoPrefix *>(splitNode(splitIndex + 1));
    ...
    // Create a new node that will replace the current node because the prefixLen could change so all keys need to be reinserted
    BTreeLeafNodeBinarySearchNoPrefix *newThisChild = new BTreeLeafNodeBinarySearchNoPrefix();
