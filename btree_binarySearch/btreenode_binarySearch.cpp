...
void BTreeNodeBinarySearch::updateHints() {
  if (numKeys <= 16) {
    return;
  }
  ...
  for (uint16_t i = 0; i < 16; i++) {
    auto slot = reinterpret_cast<PageSlotBinarySearch *>(content + i * spacing * sizeof(PageSlotBinarySearch));
    hints[i] = slot->keyHead;
  }
}

std::pair<int, int> BTreeNodeBinarySearch::getHintRange(uint32_t keyHead) {
  if (numKeys <= 16) {
