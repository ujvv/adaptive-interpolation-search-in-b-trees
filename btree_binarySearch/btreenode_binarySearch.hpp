...
// constexpr uint32_t NODE_SIZE_BINARYSEARCH = 16384;
// constexpr uint32_t NODE_SIZE_BINARYSEARCH = 32768;
// constexpr uint32_t NODE_SIZE_BINARYSEARCH = 65536;
// constexpr uint32_t NODE_SIZE_BINARYSEARCH = 131072;
...
struct BTreeNodeHeaderBinarySearch {
  ...
  uint32_t freeOffset = NODE_SIZE_BINARYSEARCH - sizeof(BTreeNodeHeaderBinarySearch);
...
private:
  ...
  void insertEntry(uint32_t position, std::span<uint8_t> key, BTreeNodeBinarySearch *childPointer);
  ...
  BTreeNodeBinarySearch *splitNode(uint32_t splitIndex, std::span<uint8_t> splitKey);
...
public:
  ...
  BTreeNodeBinarySearch *getChild(uint32_t position) {
  ...
  void overwriteChild(uint32_t position, BTreeNodeBinarySearch *newChild);
};
